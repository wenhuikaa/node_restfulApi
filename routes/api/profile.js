...
router.get("/", passport.authenticate('jwt', { session: false }), (req, res) => {
    const errors = {};
    //一定要注意找的user是引进的users表里的，所以是req.user,而不是req.body
    //populate('user', ["name", "avatar"]):指定要将users表的数据传过来
    //因为ProfileSchema模型中的属性user是ProfileSchema和UserSchema的关联
...
router.post("/", passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    //profileFields是对象，所以定义一个对象，默认为空
    const profileFields = {};
...
router.delete("/experience/:epx_id", passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    //stringObject.indexOf(searchvalue,fromindex),searchvalue 必需。规定需检索的字符串值。
    //indexOf() 方法可返回某个指定的字符串值在字符串中首次出
    Profile.findOne({ user: req.user.id })
        .then(profile => {
